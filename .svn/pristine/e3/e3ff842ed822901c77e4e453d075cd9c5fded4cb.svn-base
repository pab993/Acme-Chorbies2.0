...
<%-- 	<jstl:set var="color" value="background-color:#FFFFFF" />
	...
	<security:authorize access="hasRole('CHORBI')">
		...
		<display:column>
			...
			<jstl:choose>
				...
				<jstl:when test="${row.chorbies.contains(principal) and row.moment > fecha}">
					 <a href="event/chorbi/unRegister.do?idEvent=${row.id }"><spring:message code="event.unRegister" /></a>
				
				</jstl:when>
	
				<jstl:when test="${fn:length(row.chorbies) < row.seatsNumber and row.moment > fecha}">
	...
	<security:authorize access="hasRole('MANAGER')">
		...
		<display:column title="${eEdit}">
			<jstl:choose>
				<jstl:when test="${principal.events.contains(row) == true}">
					<a href="manag/events/edit.do?eventId=${row.id}"><spring:message code="event.edit" /></a>
				</jstl:when>
				<jstl:otherwise>
					--
				</jstl:otherwise>
		...
		<display:column title="${eventBroadcast}">
			<jstl:choose>
				<jstl:when test="${principal.events.contains(row) == true}">
					<a href="manag/events/broadcast.do?eventId=${row.id}"><spring:message code="event.broadcast" /></a>
				</jstl:when>
				<jstl:otherwise>
					--
				</jstl:otherwise>
	...
	<security:authorize access="hasRole('MANAGER')">
		...
		<div>
			<a href="manag/events/create.do"><spring:message code="manager.events.create"/></a>
		</div>
